...
type Store = { version: 2; exportedAt: string; settings: { grid: number; message: string }; workspaces: Workspace[]; activeWorkspaceId: string; widgets: Widget[] };
...
const backupSchema = z.object({
  ...
  groups: z.array(z.object({ id: z.string(), name: z.string(), color: z.string(), locked: z.boolean() })).optional(),
...
const initial: Store = {
  ...
  widgets: [
    ...
    make("todo", "오늘의 학습", 360, 40, { done: false, due: new Date().toISOString().slice(0,10), priority: "HIGH" }),
    make("note", "알고리즘 메모", 40, 270, { body: "BFS: 큐에 넣을 때 방문 처리\n시간복잡도 O(V + E)" }),
    make("bookmark", "SW EXPERT ACADEMY", 390, 270, { url: "https://swexpertacademy.com" }),
  ],
};
...
const normalizeStore = (value: unknown): Store => {
  ...
  return { version: 2, exportedAt: parsed.exportedAt, settings: { grid: parsed.settings.grid, message: parsed.settings.message || "꾸준함이 결국 실력을 만든다." }, workspaces, activeWorkspaceId, widgets: parsed.widgets.map(w => ({ ...w, groupId: null, workspaceId: w.workspaceId || MAIN_WORKSPACE, style: w.style || { borderColor: "#303b47" } })) };
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    interact(selector)
      .draggable({ allowFrom: ".drag-handle", listeners: {
        start(e) { const id = (e.target as HTMLElement).dataset.id || ""; dragIdsRef.current = selected.includes(id) ? selected : [id]; if (!selected.includes(id)) setSelected([id]); document.body.classList.add("is-dragging"); },
        move(e) {
          for (const id of dragIdsRef.current) { const target = canvasRef.current?.querySelector<HTMLElement>(`[data-id="${id}"]`); if (!target) continue; const nextX = Number(target.dataset.x || 0) + e.dx; const nextY = Math.max(0, Number(target.dataset.y || 0) + e.dy); target.style.transform = `translate(${nextX}px, ${nextY}px)`; target.dataset.x = String(nextX); target.dataset.y = String(nextY); }
        },
        end() { const positions = new Map(dragIdsRef.current.map(id => { const el = canvasRef.current?.querySelector<HTMLElement>(`[data-id="${id}"]`); return [id, { x: Number(el?.dataset.x || 0), y: Number(el?.dataset.y || 0) }] as const; })); setStore(s => ({ ...s, widgets: s.widgets.map(w => positions.has(w.id) ? { ...w, layout: { ...w.layout, ...positions.get(w.id)! }, updatedAt: now() } : w) })); document.body.classList.remove("is-dragging"); },
      } })
      .resizable({ edges: { left: true, right: true, bottom: true, top: true }, modifiers: [interact.modifiers.restrictSize({ min: { width: 240, height: 150 } })], listeners: { move(e) { const target = e.target as HTMLElement; const x = Number(target.dataset.x || 0) + e.deltaRect.left; const y = Math.max(0, Number(target.dataset.y || 0) + e.deltaRect.top); target.style.width = `${e.rect.width}px`; target.style.height = `${e.rect.height}px`; target.style.transform = `translate(${x}px, ${y}px)`; target.dataset.x = String(x); target.dataset.y = String(y); target.dataset.width = String(e.rect.width); target.dataset.height = String(e.rect.height); }, end(e) { const target = e.target as HTMLElement; updateLayout(target.dataset.id || "", { x: Number(target.dataset.x || 0), y: Number(target.dataset.y || 0), width: Number(target.dataset.width || 0), height: Number(target.dataset.height || 0) }); } } });
  ...
  const counts = useMemo(() => ({ todo: visibleWidgets.filter(w => w.type === "todo" && !w.data.done).length, bookmark: visibleWidgets.filter(w => w.type === "bookmark").length }), [visibleWidgets]);
  ...
  const beginSelection = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const start = { x: event.clientX - rect.left, y: event.clientY - rect.top }; const base = event.ctrlKey ? selected : [];
    const move = (e: PointerEvent) => { const current = { x: e.clientX - rect.left, y: e.clientY - rect.top }; const box = { x: Math.min(start.x, current.x), y: Math.min(start.y, current.y), width: Math.abs(current.x - start.x), height: Math.abs(current.y - start.y) }; setSelectionBox(box); const hits = visibleWidgets.filter(w => w.layout.x < box.x + box.width && w.layout.x + w.layout.width > box.x && w.layout.y < box.y + box.height && w.layout.y + w.layout.height > box.y).map(w => w.id); setSelected([...new Set([...base, ...hits])]); };
    const end = () => { setSelectionBox(null); window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", end); };
    if (!event.ctrlKey) setSelected([]); setSelectionBox({ x: start.x, y: start.y, width: 0, height: 0 }); window.addEventListener("pointermove", move); window.addEventListener("pointerup", end);
  };
  ...
  return <main>
    ...
    <section className="commandbar">
      ...
      <div className="actions"><button onClick={() => setModal({ type: "bookmark" })}>+ BOOKMARK</button><button onClick={() => setModal({ type: "note" })}>+ NOTE</button><button onClick={() => setModal({ type: "todo" })}>+ TODO</button></div>
    </section>
    <section className="stats" aria-label="요약">
      <div className="datetime"><span>{new Intl.DateTimeFormat("ko-KR", { month: "short", day: "2-digit", weekday: "short" }).format(clock)}</span><b>{new Intl.DateTimeFormat("ko-KR", { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }).format(clock)}</b><small>{clock.getFullYear()}</small></div><article><span>OPEN TASKS</span><b>{String(counts.todo).padStart(2,"0")}</b><i>FOCUS</i></article><article><span>BOOKMARKS</span><b>{String(counts.bookmark).padStart(2,"0")}</b><i>LINKS</i></article><article><span>SELECTED</span><b>{String(selected.length).padStart(2,"0")}</b><i>WIDGETS</i></article><div className="quote"><textarea aria-label="상단 메모" value={store.settings.message} onChange={e => setStore(s => ({ ...s, settings: { ...s.settings, message: e.target.value } }))} placeholder="기억할 문장을 적어두세요."/></div>
    </section>
    <section className="canvas-wrap"><div className="canvas-label"><div className="workspace-tabs"><button className={settingsOpen ? "active settings-tab" : "settings-tab"} aria-label="설정" onClick={() => { setSettingsOpen(true); setSelected([]); }}>⚙</button>{store.workspaces.map(ws => <button key={ws.id} className={!settingsOpen && ws.id === store.activeWorkspaceId ? "active" : ""} onClick={() => { setSettingsOpen(false); setSelected([]); setStore(s => ({ ...s, activeWorkspaceId: ws.id })); }}>{ws.name}</button>)}</div></div>
      {settingsOpen ? <div className="settings-space"><aside><div className="settings-title">PREFERENCES</div><button className="active">워크스페이스</button><button disabled>화면 및 테마</button><button disabled>데이터 관리</button></aside><section><div className="settings-heading"><div><small>SETTINGS</small><h2>워크스페이스 관리</h2><p>워크스페이스를 추가하거나 이름을 바꾸고 삭제할 수 있습니다.</p></div><button onClick={() => { const name = prompt("새 워크스페이스 이름", `SPACE ${store.workspaces.length + 1}`); if (!name) return; const id = crypto.randomUUID(); setStore(s => ({ ...s, workspaces: [...s.workspaces, { id, name }], activeWorkspaceId: id })); }}>＋ 새 워크스페이스</button></div><div className="workspace-manager">{store.workspaces.map(ws => <div className="workspace-row" key={ws.id}><div><span>{ws.name.slice(0,1).toUpperCase()}</span><strong>{ws.name}</strong><small>{store.widgets.filter(w => w.workspaceId === ws.id).length} widgets</small></div><div><button onClick={() => { const name = prompt("워크스페이스 이름 변경", ws.name); if (!name) return; setStore(s => ({ ...s, workspaces: s.workspaces.map(w => w.id === ws.id ? { ...w, name } : w) })); }}>이름 변경</button><button className="danger" disabled={store.workspaces.length === 1} onClick={() => { if (!confirm(`'${ws.name}' 워크스페이스와 포함된 위젯을 삭제할까요?`)) return; setStore(s => { const remaining = s.workspaces.filter(w => w.id !== ws.id); return { ...s, workspaces: remaining, activeWorkspaceId: s.activeWorkspaceId === ws.id ? remaining[0].id : s.activeWorkspaceId, widgets: s.widgets.filter(w => w.workspaceId !== ws.id) }; }); }}>삭제</button></div></div>)}</div></section></div> : <div className="canvas" ref={canvasRef} onPointerDown={beginSelection} style={{ height: Math.max(650, ...visibleWidgets.map(w => w.layout.y + w.layout.height + 80)) }}>
        {selectionBox && <span className="selection-box" style={{ left: selectionBox.x, top: selectionBox.y, width: selectionBox.width, height: selectionBox.height }}/>} 
        {visibleWidgets.map(w => <article key={w.id} data-id={w.id} data-x={w.layout.x} data-y={w.layout.y} data-width={w.layout.width} data-height={w.layout.height} className={`widget ${selected.includes(w.id) ? "selected" : ""}`} style={{ transform: `translate(${w.layout.x}px, ${w.layout.y}px)`, width: w.layout.width, height: w.layout.height, zIndex: w.layout.zIndex, borderColor: w.style.borderColor }} onClick={e => { if ((e.target as HTMLElement).closest("button,a,input,textarea")) return; if (e.ctrlKey) setSelected(s => s.includes(w.id) ? s.filter(id => id !== w.id) : [...s, w.id]); else if (!selected.includes(w.id)) setSelected([w.id]); }}>
          <div className="widget-top drag-handle"><Icon type={w.type}/><span>{w.type.toUpperCase()}</span><div className="widget-tools"><button aria-label="수정" onPointerDown={e=>e.stopPropagation()} onClick={() => setModal({ type: w.type, widgetId: w.id })}>✎</button><button aria-label="복제" onPointerDown={e=>e.stopPropagation()} onClick={() => setStore(s => ({...s,widgets:[...s.widgets,{...w,id:crypto.randomUUID(),layout:{...w.layout,x:w.layout.x+20,y:w.layout.y+20}}]}))}>⧉</button><button aria-label="삭제" onPointerDown={e=>e.stopPropagation()} onClick={() => remove(w.id)}>×</button></div></div>
          ...
          </div><span className="resize-corner"/>
        </article>)}
      </div>}
    </section>
