...
function DashboardCat({
  ...
  useEffect(() => {
    ...
    const clampToViewport = (x: number, width = 96) => {
      const main = document.querySelector("main")?.getBoundingClientRect();
      if (!main) return Math.max(8, x);
      const viewportLeft = Math.max(0, main.left) - main.left + 8;
      const viewportRight =
        Math.min(document.documentElement.clientWidth, main.right) -
        main.left -
        width -
        8;
      return Math.max(viewportLeft, Math.min(Math.max(viewportLeft, viewportRight), x));
    };
    const locate = (
      element: Element,
      home = false,
      ...
      return {
        x: clampToViewport(
          rect.left -
            main.left +
            (home ? rect.width + 18 : rect.width * landingRatio - 48),
        ),
        y: home
    ...
    const availableWidgets = (excludedId?: string): CatTarget[] =>
      Array.from(document.querySelectorAll<HTMLElement>(".widget .widget-top"))
        ...
        .map(({ element, id }) => ({
          ...locate(element, false, 0.1 + Math.random() * 0.8),
          platformId: id,
        }));
    const travel = async (target: CatTarget) => {
      ...
      if (target.platformId === platformRef.current && distance < 1) {
        destinationRef.current = "";
        return;
      }
    ...
    const chooseNext = async (excludedIds: string[] = []) => {
      ...
      if (platforms.length === 0 && platformRef.current === "workspace-bar")
        return;
      const target =
        platformRef.current !== "workspace-bar" &&
    ...
    const changeWorkspace = async () => {
      window.clearTimeout(timer);
      if (platformRef.current !== "workspace-bar") await travel(workspaceBar());
      if (cancelled) return;
      timer = window.setTimeout(async () => {
        await chooseNext();
        if (!cancelled) schedule();
    ...
    const keepInViewport = () => {
      ...
      resizeFrame = window.requestAnimationFrame(() => {
        if (cancelled) return;
        const targetId = destinationRef.current || platformRef.current;
        const targetElement =
          targetId === "workspace-bar"
            ? document.querySelector(".canvas-label")
            : Array.from(document.querySelectorAll<HTMLElement>(".widget .widget-top")).find(
                (element) =>
                  element.closest<HTMLElement>(".widget")?.dataset.id === targetId,
              );
        if (targetElement) {
          ...
          settle(
            targetId === "workspace-bar"
              ? workspaceBar()
              : { ...locate(targetElement, false, 0.5), platformId: targetId },
            poseRef.current === "jump" ? "sit" : poseRef.current,
          );
          return;
        }
        const next = {
          ...positionRef.current,
          x: clampToViewport(positionRef.current.x),
        };
        positionRef.current = next;
        setPosition(next);
      });
    ...
    const followWidget = () => {
      if (!draggingRef.current) {
        ...
        if (main && widget) {
          ...
          const viewportLeft = Math.max(0, main.left) - main.left;
          const viewportRight =
            Math.min(document.documentElement.clientWidth, main.right) -
            main.left -
            128;
          const next = {
            x: Math.max(
              viewportLeft,
              Math.min(Math.max(viewportLeft, viewportRight), rawX),
            ),
...
export default function Home() {
  ...
  return (
    <main data-theme={store.settings.theme}>
      ...
      />
      <DashboardChimp
        enabled={store.settings.chimpEnabled && !settingsOpen}
